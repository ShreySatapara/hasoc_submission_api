...
def add_team():
    try:
        if request.method == 'POST':
            ...
            data = db.participants.insert_one(
                {'_id': team_name, 'email': email, 'password_hash': password_hash})
            return Response(
    ...
    try:
        if request.method == 'POST':
            ...
            if(team_name != '' and password != ''):
                data = list(db.participants.find({'_id': team_name}))
                if(len(data) == 0):
        ...
        if request.method == 'POST':
            ...
            if(len(data) > 0):
                ...
                if(bcrypt.check_password_hash(db_password_hash, password)):
                    ...
                    db.participants.update_one({'_id': team_name}, {'$set': {'password_hash': password_hash}})
